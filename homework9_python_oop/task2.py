...
class HardworkingTrainee(Trainee):
    def do_homework(self) -> None:
        ...
        Using score method from Trainee
        """
        self.score += 2
...
class Cohort:
    ...
    def conduct_lecture(self) -> None:
        """calls method 'visit_lecture' for each trainee in group"""
        for trainee in self.trainees:
            trainee.visit_lecture()

    def get_passing_students(self) -> list[Trainee]:
        """
        return list of all trainees for whom
        method 'is_passing' returns True
          
        """
        return [pass_trainee for pass_trainee in self.trainees if pass_trainee.is_passing()]
